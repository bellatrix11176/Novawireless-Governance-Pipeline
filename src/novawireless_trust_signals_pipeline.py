...
TRUST SIGNAL HEALTH PIPELINE (Lab-Aligned, Enhanced v3)
...
def build_integrity_flags(df: pd.DataFrame, cfg: IntegrityConfig) -> pd.DataFrame:
    ...
    if {"credit_type", "credit_authorized"}.issubset(df.columns):
        # Soft signal only — recorded for analysis but does not trigger quarantine
...
def run_integrity_gate(df, out_dirs, cfg, dupe_policy):
    # Scope duplicate detection to call_id within each source file,
    # since call IDs may reset or overlap across monthly files
    if "_source_file" in df.columns:
        df = df.copy()
        df["_composite_key"] = df["call_id"].astype(str) + "|" + df["_source_file"].astype(str)
...
def compute_trust_signals(df):
    ...
    # ── Transcript: scripted window-management language detection ──────────────────
    # The paper identifies specific scripted phrases as the primary mechanism driving
    # 31-60d callbacks — reps are trained to use language that manages the customer's
    # expectation past the FCR measurement window without actually resolving the issue.
    # These patterns are detectable via simple string matching on transcript_text.
    #
    # scripted_window_flag: 1 if any window-management phrase detected in transcript.
    # scripted_phrase_count: number of distinct phrase patterns matched (0–N).
    # transcript_word_count: proxy for resolution effort (longer ≠ better, but
    #   very short transcripts on complex calls suggest early termination).
    _SCRIPTED_PATTERNS = [
        # Non-capturing groups (?:...) throughout — avoids pandas str.contains warning.
        #
        # These patterns are calibrated to this dataset's simulation transcripts, where
        # FCR gaming manifests as performative honesty language rather than the billing-cycle
        # deflection phrases common in real call center transcripts. The key tells:
        #
        # 1. Temporal deflection — promising resolution in a future cycle rather than now.
        #    Occurs in 100% of gamed_metric transcripts, 0% of clean.
        r"(?:call|hear from us) (?:back )?next month",
        r"have you call back next",
        r"next month.{0,30}(?:resolved|fixed|taken care)",
        #
        # 2. Performative honesty with no resolution — "I'm being straight with you"
        #    language that acknowledges the issue without fixing it, designed to close
        #    the call with a satisfied-customer label. 100% gamed_metric, 0% clean.
        ...
        #
        # 3. False prior-resolution acknowledgement — admits previous calls were
        #    marked resolved without being fixed (escalation cover language).
        ...
        #
        # 4. Not-able-to-override deflection — billing-cycle language adapted to
        #    this dataset. 34% gamed_metric, 100% unresolvable_clean.
    ...
    if "transcript_text" in out.columns:
        ...
        # Use re.search via apply — avoids pandas str.contains capture-group warning
        # while correctly matching anywhere in the transcript text.
        _compiled = [_re.compile(p, _re.IGNORECASE) for p in _SCRIPTED_PATTERNS]
        out["scripted_phrase_count"] = sum(
            txt.apply(lambda t: int(bool(cp.search(t)))) for cp in _compiled
        )
        out["scripted_window_flag"] = (out["scripted_phrase_count"] > 0).astype(int)
        out["transcript_word_count"] = txt.str.split().str.len().fillna(0).astype(int)
    else:
        out["scripted_window_flag"]  = 0
        out["scripted_phrase_count"] = 0
        out["transcript_word_count"] = 0
    # detection_signal_density: fraction of detection flags firing per call.
    # scripted_window_flag is included here so transcript-based gaming detection
    # feeds the same composite signal as the structural fraud/gaming flags.
    det_cols = [c for c in DETECTION_FLAG_COLS if c in out.columns] + ["scripted_window_flag"]
    det = list(dict.fromkeys(det_cols))  # deduplicate, preserve order
    ...
    # A $25 credit on a $27/month plan is qualitatively different from a $5 courtesy credit.
    # Ratio > 0.5 (credit exceeds half the monthly bill) indicates substantive appeasement,
    # not a token gesture — add up to 0.2 to credit_risk_score, capped so score stays in [0,1].
    ...
    # rep_drift_score: composite of gaming propensity, burnout, and aware gaming.
    # rep_aware_gaming = rep consciously gamed the metric on this call (binary flag in data).
    # Aware gaming is weighted highest (0.45) because it is deliberate, not structural —
    # a rep who knows they are gaming is a different risk profile than one optimizing
    # toward an incentive they don't fully understand.
    # Weights: aware_gaming=0.45, gaming_propensity=0.35, burnout=0.20. Sum = 1.0.
    aware = _safe_coerce(out, "rep_aware_gaming") if "rep_aware_gaming" in out.columns else pd.Series(0.0, index=out.index)
...
def summarize_by_rep(df, min_calls=1):
    ...
    agg = df.groupby("rep_id", dropna=False).agg(
        ...
        repeat_31_60d_rate=("repeat_contact_31_60d", lambda s: _coerce_flag(s).mean()) if "repeat_contact_31_60d" in df.columns else ("call_id", lambda s: 0.0),
        ...
        aware_gaming_rate=("rep_aware_gaming", lambda s: _coerce_flag(s).mean()) if "rep_aware_gaming" in df.columns else ("call_id", lambda s: np.nan),
        scripted_phrase_rate=("scripted_window_flag", lambda s: _coerce_flag(s).mean()) if "scripted_window_flag" in df.columns else ("call_id", lambda s: np.nan),
    ).reset_index()
    agg["resolution_gap"] = (agg["proxy_resolution_rate"] - agg["true_resolution_rate"]).round(4)
    # Rep-level DAR (Delayed Adverse Rate) — paper formula F/D applied per rep.
    # This is the individual-rep version of the dataset-level DAR: identifies
    # which specific reps are driving 31-60d callbacks after labeled resolution.
    # Computed here rather than via agg() because it requires row-level logic
    # (filtering to resolved calls only, then checking 31-60d flag on that subset).
    if "resolution_flag" in df.columns and "repeat_contact_31_60d" in df.columns:
        def _rep_dar(g):
            ...
            return round(F / D, 6)
        rep_dar = df.groupby("rep_id", dropna=False).apply(_rep_dar).reset_index()
...
def summarize_by_scenario(df):
    ...
    agg = df.groupby("scenario", dropna=False).agg(
        ...
        trust_score_avg=("call_trust_score","mean"),
        outcome_risk_avg=("outcome_risk_score","mean"),
        scripted_rate=("scripted_window_flag", lambda s: _coerce_flag(s).mean()) if "scripted_window_flag" in df.columns else ("call_id", lambda s: np.nan),
    ).reset_index()
    agg["pct_of_total"] = (agg["calls"] / max(len(df),1) * 100).round(2)
    agg["resolution_gap"] = (agg["proxy_resolution_rate"] - agg["true_resolution_rate"]).round(4)
    # Scenario-level DOV (Durable Outcome Validation) — per-scenario proxy accuracy
    # against true_resolution. Identifies which specific call types are degrading
    # the measurement environment, not just the system as a whole.
    # DOV = 1 - accuracy (accuracy = fraction where proxy label == true label).
    # Higher = worse, consistent with all other governance signals.
    if "resolution_flag" in df.columns and "true_resolution" in df.columns:
        ...
        scen_dov = df.groupby("scenario", dropna=False).apply(_scen_dov).reset_index()
...
#
# DAR — Delayed Adverse Rate        DAR_raw = F / D, then N↑(x; L, H)
# DRL — Downstream Remediation Load DRL_raw = JS(p ∥ q), then N↑(x; L, H)
# DOV — Durable Outcome Validation  DOV = clamp((A_base - A_cur) / (A_base + ε), 0, 1)
# POR — Proxy Overfit Ratio         POR_raw = clamp(ΔP/(ΔT+ε), 0, K), norm to [0,1]
# TER — Terminal Exit Rate          churn rate among proxy-resolved calls (diagnostic)
# SII — System Integrity Index      100*(0.30*DAR + 0.20*DRL + 0.25*DOV + 0.25*POR)
#                                   DOV gate: if DOV >= τ → SII_gated = 100
#
# Normalization bounds — set for early detection (signals climb before catastrophe):
#   DAR:  L=0.05  H=0.40   (starts moving at 5% repeat rate, saturates at 40%)
#   DRL:  L=0.02  H=0.30   (JS divergence; 0.02 = noise floor, 0.30 = serious drift)
#   POR:  K=5.0             (proxy improving 5× faster than true outcomes = saturation)
#
# SII thresholds (from paper):
#   SII >= 60  → VETO:  Halt AI optimization. Re-audit measurement environment.
#   SII 30-60  → WATCH: Drift detected. Human review before next optimization cycle.
#   DOV gate τ = 0.50   (DOV >= 0.50 forces SII = 100 regardless of other signals)

PAPER_SIGNAL_DEFAULTS = {
...
def _norm_up(x: float, L: float, H: float) -> float:
    """N↑(x; L, H) — Higher-Is-Worse Normalization.

    Paper (Appendix A):
        N↑(x; L, H) = clamp((x - L) / (H - L), 0, 1)

    Converts a raw signal value into a standardized risk score on [0, 1].
        x ≤ L → 0.0  (at or below the lower bound: no detected risk)
        x ≥ H → 1.0  (at or above the upper bound: maximum risk)
        L < x < H → linear interpolation between 0 and 1

    L and H are calibration bounds specific to each signal (DAR, DRL, POR).
    They encode the domain knowledge of what constitutes negligible vs.
    saturating risk for that particular measurement."""
    if H <= L:
        return 0.0
...
def _js_divergence(p: np.ndarray, q: np.ndarray) -> float:
    ...
    Used by DRL to measure distributional drift between scenario frequency
    distributions across two time periods.

    JS(p ∥ q) = 0.5 · KL(p ∥ m) + 0.5 · KL(q ∥ m)
    where m = 0.5 · (p + q)  (the mixture distribution)
    and   KL(a ∥ b) = Σ a_i · log(a_i / b_i)  (Kullback-Leibler divergence)

    Properties:
        Symmetric: JS(p ∥ q) == JS(q ∥ p)
        Bounded:   JS ∈ [0, ln2 ≈ 0.693]
        JS = 0 when p and q are identical distributions
        JS = ln2 when p and q have completely disjoint support

    Both p and q are normalized to sum to 1.0 before computation.
    Returns 0.0 if either distribution is all-zero (no data for that period).
    log(0) terms are excluded via masking (only positions where both a > 0
    and b > 0 contribute to the KL sum)."""
    ...
    q = q / q.sum()
    m = 0.5 * (p + q)
    # KL(p||m) + KL(q||m), guarded against log(0)
    def _kl(a, b):
        mask = (a > 0) & (b > 0)
...
def _compute_dar(df: pd.DataFrame, cfg: dict) -> Tuple[float, float]:
    ...
    Where:
        F = number of labeled-resolved calls that received a repeat contact
            within the 31–60 day window (adverse recurrence after labeled success)
        D = total number of calls labeled resolved (proxy success denominator)
        N↑ = higher-is-worse normalization: clamp((x - L) / (H - L), 0, 1)

    Implementation note:
        DAR_raw is computed as F / (D + ε) rather than F / D.
        ε (EPSILON ≈ 1e-9) is a division guard for the D=0 edge case only.
        When D > 0 — which is always true at this point due to the early return
        above — the ε term is numerically negligible and the result is
        mathematically identical to the paper formula F / D.
    ...
    # Paper: DAR_raw = F / D  (ε guard is inactive when D > 0; see docstring)
    DAR_raw  = F / (D + eps)
    DAR_norm = _norm_up(DAR_raw, cfg["DAR_L"], cfg["DAR_H"])
    return round(DAR_raw, 6), round(DAR_norm, 6)

def _compute_drl(df: pd.DataFrame, cfg: dict) -> Tuple[float, float, int]:
    ...
    Where:
        p        = scenario frequency distribution in the current (last) month
        q        = scenario frequency distribution in the baseline (first) month
        JS(p∥q)  = Jensen-Shannon divergence — a symmetric, bounded measure of
                   distributional distance; JS ∈ [0, ln2 ≈ 0.693]
        N↑       = higher-is-worse normalization: clamp((x - L) / (H - L), 0, 1)

    Interpretation (from paper):
        Detects distributional drift in post-success workload. Rising DRL means
        the mix of call scenarios following labeled-resolved interactions is shifting
        away from the baseline — a signal that the proxy label is attracting a
        different population of downstream problems over time.

    Requires ≥ 2 months of data. Returns (0.0, 0.0, n_months) when only one
    month is available — this is not a zero-drift finding, it is insufficient
    data to compute drift at all.

    Returns (DRL_raw, DRL_normalized, n_months)."""
    ...
    if tmp["_month"].nunique() < 2:
        return 0.0, 0.0, tmp["_month"].nunique()
    ...
    return round(DRL_raw, 6), round(DRL_norm, 6), tmp["_month"].nunique()
...
def _compute_dov(df: pd.DataFrame, cfg: dict) -> float:
    ...
    Where:
        A_base = accuracy of the proxy label against the durable outcome
                 in the baseline (first) month
        A_cur  = accuracy of the proxy label against the durable outcome
                 in the current (last) month
        ε      = EPSILON ≈ 1e-9, guards against division by zero when A_base = 0
        clamp  = clamp(x, 0, 1) = min(1, max(0, x))

    Accuracy is defined as: fraction of calls where resolution_flag == true_resolution.
    A_base > A_cur → proxy validity is decaying → DOV > 0 (worse).
    A_base ≤ A_cur → no decay detected → DOV = 0.

    Interpretation (from paper):
        Measures decay in predictive validity. A rising DOV means the proxy label
        is becoming a progressively worse predictor of durable outcomes — the
        system is optimizing a signal that is losing its connection to reality.

    DOV gate: if DOV ≥ τ (default τ = 0.50), SII is forced to 100 regardless
    of other signal values. This is the paper's hard veto on severe validity decay.

    Single-month fallback: when only one month of data is available, A_base is
    set to 1.0 (perfect accuracy) and A_cur is the observed accuracy. This gives
    a conservative lower-bound sensitivity reading rather than returning 0.
    ...
    if "call_date" not in df.columns:
        # No time dimension — compute single-period accuracy decay vs perfect baseline
        proxy = _coerce_flag(df["resolution_flag"])
        true  = _coerce_flag(df["true_resolution"])
        A_base = 1.0  # perfect accuracy as fallback baseline
    ...
    tmp = tmp.dropna(subset=["_month"])
    if tmp["_month"].nunique() < 2:
        # Single month — use mean accuracy vs perfect baseline for sensitivity
...
def _compute_por(df: pd.DataFrame, cfg: dict) -> Tuple[float, float, int]:
    ...
    Where:
        ΔP      = P_cur - P_base  (change in proxy resolution rate, first→last month)
        ΔT      = T_cur - T_base  (change in true resolution rate, first→last month)
        ε       = EPSILON ≈ 1e-9, guards against division by zero when ΔT = 0
        K       = POR_K (default 5.0) — saturation ceiling; POR_raw is clamped
                  at K, meaning "proxy improving ≥5× faster than true outcomes"
                  is treated as maximum signal regardless of exact magnitude
        clamp   = clamp(x, 0, 1) = min(1, max(0, x))

    Interpretation (from paper):
        Measures acceleration imbalance between proxy and durable outcome improvement.
        POR_raw = 1.0 means proxy and true outcomes are improving at the same rate
        (no imbalance). POR_raw > 1.0 means the proxy is improving faster than
        true outcomes — the gap is widening. The normalization maps this so that
        POR = 0 at POR_raw ≤ 1 (no excess acceleration) and POR = 1 at
        POR_raw = K (maximum detected imbalance).

    Implementation note:
        ΔP / (ΔT + ε) rather than ΔP / ΔT. The ε term is a division guard for
        the ΔT = 0 case (true outcomes flat while proxy improves). When ΔT ≠ 0
        the ε term is numerically negligible and the result is mathematically
        identical to the paper formula.

    Requires ≥ 2 months of data to compute ΔP and ΔT. Returns (0.0, 0.0, n_months)
    when only one month is available — not a zero-imbalance finding.

    Returns (POR_raw, POR_normalized, n_months)."""
    eps = cfg["EPSILON"]
    K   = cfg["POR_K"]
    if "resolution_flag" not in df.columns or "true_resolution" not in df.columns:
        return 0.0, 0.0, 0
    ...
    return round(POR_raw, 6), round(POR_norm, 6), tmp["_month"].nunique()

def _compute_ter(df: pd.DataFrame) -> Optional[float]:
    """TER — Terminal Exit Rate.

    Paper (Appendix A):
        TER = churn rate among proxy-resolved calls

    Where:
        Proxy-resolved calls = calls where resolution_flag == 1 (labeled success
                               by the proxy KPI)
        Churn rate           = fraction of those customers who subsequently churned
                               (customer_is_churned == 1)

    Interpretation (from paper):
        TER is a diagnostic signal, not a component of SII. Its significance is
        not its absolute value but its relationship to the baseline churn rate.
        When TER ≈ baseline churn, the proxy resolution label carries no retention
        signal — a customer labeled "resolved" is no more likely to stay than any
        other customer. This is the paper's evidence that the proxy label has
        decoupled from durable outcomes.

        Paper finding: TER = 27.6% ≈ baseline churn 27.63% — statistically
        indistinguishable. The resolution flag has an odds ratio of 0.99 against
        churn (p = .78), confirming it is no better than a coin flip as a
        predictor of customer retention.

    Returns TER as a float in [0, 1], or None if required columns are absent."""
    ...
    if len(resolved_calls) == 0:
        return None
    ter = float(_coerce_flag(resolved_calls["customer_is_churned"]).mean())
    return round(ter, 6)

def _compute_sii(DAR: float, DRL: float, DOV: float, POR: float,
                 cfg: dict) -> Tuple[float, float, str]:
    """SII — System Integrity Index.

    Paper (Appendix A):
        w_DAR + w_DRL + w_DOV + w_POR = 1
        SII       = 100 · (w_DAR·DAR + w_DRL·DRL + w_DOV·DOV + w_POR·POR)
        SII_gated = 100  if DOV ≥ τ
                    SII  otherwise

    Default weights (from paper):
        w_DAR = 0.30,  w_DRL = 0.20,  w_DOV = 0.25,  w_POR = 0.25

    Where:
        All four component signals (DAR, DRL, DOV, POR) are pre-normalized to
        [0, 1] before aggregation. Higher = worse across all signals.
        The ×100 scale produces SII ∈ [0, 100].

    DOV gate:
        If DOV ≥ τ (default τ = 0.50), SII_gated is forced to 100 regardless
        of the other signal values. This is an unconditional veto: severe decay
        in predictive validity overrides all other measurements because the
        measurement environment itself can no longer be trusted.

    Thresholds (from paper):
        SII_gated ≥ 60  → VETO:  Halt AI optimization. Re-audit measurement
                                  environment before any further retraining.
        SII_gated 30–60 → WATCH: Drift detected. Human review required before
                                  next optimization cycle.
        SII_gated < 30  → OK:    Measurement environment within governance bounds.

    Interpretation (from paper):
        SII is not a performance score. It is a velocity regulator — a veto
        condition on unchecked proxy acceleration. When SII rises while proxy
        KPIs improve, the system is optimizing against a signal that is
        diverging from durable outcomes.

    Returns (SII_raw, SII_gated, status) where status ∈ {'VETO', 'WATCH', 'OK'}."""
    w = cfg["SII_weights"]
    assert abs(sum(w.values()) - 1.0) < 1e-6, f"SII weights must sum to 1.0, got {sum(w.values())}"
    SII_raw   = 100.0 * (w["DAR"]*DAR + w["DRL"]*DRL + w["DOV"]*DOV + w["POR"]*POR)
...
def compute_paper_signals(df: pd.DataFrame,
                          cfg: Optional[dict] = None) -> dict:
    """Compute all six Aulabaugh (2026) governance signals from call-level data.

    Returns a dict with keys:
      DAR, DRL, DOV, POR, TER, SII  (each a sub-dict with raw/normalized/metadata)
      summary: { SII_gated, status, weights, bounds, thresholds }
    """
    if cfg is None:
        cfg = PAPER_SIGNAL_DEFAULTS.copy()

    DAR_raw, DAR_norm              = _compute_dar(df, cfg)
    DRL_raw, DRL_norm, drl_n_months = _compute_drl(df, cfg)
    DOV_norm                        = _compute_dov(df, cfg)     # already [0,1]
    POR_raw, POR_norm, por_n_months = _compute_por(df, cfg)
    TER               = _compute_ter(df)
    SII_raw, SII_gated, status = _compute_sii(DAR_norm, DRL_norm, DOV_norm, POR_norm, cfg)

    # Baseline churn for TER context
    baseline_churn = None
    if "customer_is_churned" in df.columns:
        baseline_churn = round(float(_coerce_flag(df["customer_is_churned"]).mean()), 6)
    ...
    result = {
        ...
        "DRL": {
            ...
            "n_months_available": drl_n_months,
            "requires_multi_month": True,
            "note": "Requires ≥2 months of data. Value is 0.0 when only 1 month available — not a zero-drift finding." if drl_n_months < 2 else None,
        },
        "DOV": {
        ...
        "POR": {
            ...
            "raw": POR_raw,
            "normalized": POR_norm,
            "formula": "clamp(ΔP/(ΔT+ε), 0, K) then clamp((raw-1)/(K-1), 0, 1)",
            "K": cfg["POR_K"],
            "n_months_available": por_n_months,
            "requires_multi_month": True,
            "note": "Requires ≥2 months of data. Value is 0.0 when only 1 month available — not a zero-drift finding." if por_n_months < 2 else None,
        },
        "TER": {
            "full_name": "Terminal Exit Rate",
            "value": TER,
            "baseline_churn": baseline_churn,
            "delta_vs_baseline": round(TER - baseline_churn, 6) if TER is not None and baseline_churn is not None else None,
            "formula": "churn rate among proxy-resolved calls",
            "note": "Diagnostic only — not a SII component. TER ≈ baseline churn = proxy carries no retention signal.",
        },
        "SII": {
            ...
            "formula": "100 * (0.30*DAR + 0.20*DRL + 0.25*DOV + 0.25*POR); DOV>=τ → 100",
        ...
        "summary": {
            ...
            "status": status,
            "component_scores": {
                "DAR": DAR_norm, "DRL": DRL_norm, "DOV": DOV_norm, "POR": POR_norm
            },
            "TER": TER,
            "baseline_churn": baseline_churn,
        },
...
def format_paper_signals_text(signals: dict) -> List[str]:
    ...
    if sii["DOV_gate_applied"]:
        lines.append(f"  ⚠ DOV gate triggered (DOV={dov['normalized']:.4f} >= τ={dov['DOV_gate_tau']}) → SII forced to 100")
    lines += [
        "",
    ...
    if ter["value"] is not None:
        ...
        lines.append(f"    Proxy-resolved churn: {ter['value']:.4f}  |  Baseline churn: {ter.get('baseline_churn', 'N/A')}{delta_str}")
        if ter["value"] is not None and ter.get("baseline_churn") is not None:
            if abs(ter["value"] - ter["baseline_churn"]) < 0.02:
                lines.append("    → TER ≈ baseline: proxy resolution flag carries NO retention signal.")
    else:
    ...
    lines += [
        ...
        f"    DOV gate τ={dov['DOV_gate_tau']}  |  SII VETO≥{sii['thresholds']['veto']}  WATCH≥{sii['thresholds']['watch']}",
    ]
    return lines
...
def compute_churn_by_trust_decile(df):
    ...
    spec = {"calls":("call_id","count"), "trust_score_min":("call_trust_score","min"),
            ...
            "proxy_resolution_rate":("resolution_flag", lambda s: pd.to_numeric(_coerce_flag(s), errors="coerce").mean()),
            "true_resolution_rate":("true_resolution",  lambda s: pd.to_numeric(_coerce_flag(s), errors="coerce").mean())}
    if "repeat_contact_30d" in out.columns:
        spec["repeat_30d_rate"] = ("repeat_contact_30d", lambda s: _coerce_flag(s).mean())
    dec = out.groupby("trust_decile").agg(**spec).reset_index()
    dec["proxy_resolution_rate"] = pd.to_numeric(dec["proxy_resolution_rate"], errors="coerce")
    dec["true_resolution_rate"]  = pd.to_numeric(dec["true_resolution_rate"],  errors="coerce")
...
def chart_scenario_drift_heatmap(scenario_summary, fig_dir):
    ...
    _apply_dark_style()
    cols = ["proxy_true_gap_rate","bandaid_rate","detection_density_avg","outcome_risk_avg","credit_applied_rate","scenario_dov","scripted_rate"]
...
def chart_rep_dar_ranking(rep_summary, fig_dir):
    """Rep-level DAR bar chart — names the reps driving 31-60d callbacks.

    Rep DAR = F/D per rep (paper formula applied individually).
    Higher = worse. Shows which reps are responsible for the window-management
    gaming identified in the aggregate DAR signal."""
    if rep_summary.empty or "rep_dar" not in rep_summary.columns: return
    data = rep_summary.dropna(subset=["rep_dar"]).copy()
    if len(data) < 2: return
    _apply_dark_style()
    # Show worst 20 reps by DAR, minimum 20 calls
    data = data[data["calls"] >= 20].sort_values("rep_dar", ascending=False).head(20)
    if data.empty: return
    fig, ax = plt.subplots(figsize=(12, max(5, 0.4 * len(data) + 1)))
    colors = [CHART_WARN if v > 0.35 else "#ffd93d" if v > 0.20 else CHART_ACCENT
              for v in data["rep_dar"]]
    bars = ax.barh(range(len(data)), data["rep_dar"], color=colors, alpha=0.85, edgecolor="none")
    ax.set_yticks(range(len(data)))
    ax.set_yticklabels(data["rep_id"], fontsize=8)
    ax.set_xlabel("Rep DAR (F/D) — 31-60d repeat contact rate among resolved calls")
    ax.set_title("Rep-Level DAR Ranking — Who Is Driving Window-Management Gaming?\n"
                 "(Top 20 reps with ≥20 calls; red = DAR > 0.35)")
    # Reference line: dataset-level DAR
    dataset_dar = data["rep_dar"].median()
    ax.axvline(dataset_dar, color=CHART_FG, ls="--", lw=1.2, alpha=0.6,
               label=f"Median DAR: {dataset_dar:.3f}")
    ax.legend(facecolor="#16213e", edgecolor=CHART_FG, fontsize=9)
    ax.grid(True, axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(fig_dir / "rep_dar_ranking.png", dpi=CHART_DPI)
    plt.close()


def chart_scenario_dov(scenario_summary, fig_dir):
    """Scenario-level DOV bar chart — which call types are corrupting the labels.

    scenario_dov = 1 - accuracy = fraction of calls where proxy != true_resolution.
    Higher = worse. Identifies which scenarios are contributing most to measurement
    environment degradation."""
    if scenario_summary.empty or "scenario_dov" not in scenario_summary.columns: return
    data = scenario_summary.dropna(subset=["scenario_dov"]).sort_values("scenario_dov", ascending=True)
    if data.empty: return
    _apply_dark_style()
    fig, ax = plt.subplots(figsize=(12, max(5, 0.5 * len(data) + 1)))
    colors = [CHART_WARN if v > 0.5 else "#ffd93d" if v > 0.2 else CHART_ACCENT
              for v in data["scenario_dov"]]
    ax.barh(range(len(data)), data["scenario_dov"], color=colors, alpha=0.85, edgecolor="none")
    # Overlay resolution gap
    if "resolution_gap" in data.columns:
        ax2 = ax.twiny()
        ax2.plot(data["resolution_gap"], range(len(data)), "D", color="#a78bfa",
                 ms=8, alpha=0.8, label="Resolution Gap (proxy-true)")
        ax2.set_xlabel("Resolution Gap (proxy − true rate)", color="#a78bfa")
        ax2.legend(loc="lower right", facecolor="#16213e", edgecolor=CHART_FG, fontsize=9)
    ax.set_yticks(range(len(data)))
    ax.set_yticklabels(data["scenario"], fontsize=9)
    ax.set_xlabel("Scenario DOV — Proxy Label Error Rate (higher = worse)")
    ax.set_title("Scenario-Level DOV — Which Call Types Are Corrupting the Measurement Environment?")
    ax.grid(True, axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(fig_dir / "scenario_dov.png", dpi=CHART_DPI)
    plt.close()


def chart_scripted_language(df, scenario_summary, fig_dir):
    """Scripted window-management language detection chart.

    Shows the rate of scripted phrase detection by scenario, compared to
    the 31-60d callback rate. High scripted language + high DAR = the
    paper's primary mechanism confirmed in the transcript data."""
    if "scripted_window_flag" not in df.columns: return
    if "scenario" not in df.columns: return
    _apply_dark_style()
    scen_scripted = df.groupby("scenario").agg(
        scripted_rate=("scripted_window_flag", lambda s: _coerce_flag(s).mean()),
        repeat_31_60d=("repeat_contact_31_60d", lambda s: _coerce_flag(s).mean())
            if "repeat_contact_31_60d" in df.columns else ("scripted_window_flag", lambda s: np.nan),
        calls=("call_id", "count"),
    ).reset_index().sort_values("scripted_rate", ascending=True)

    fig, ax1 = plt.subplots(figsize=(12, max(5, 0.5 * len(scen_scripted) + 1)))
    y = np.arange(len(scen_scripted))
    ax1.barh(y, scen_scripted["scripted_rate"], color=CHART_WARN, alpha=0.75,
             label="Scripted Language Rate", edgecolor="none")
    ax1.set_xlabel("Rate")
    ax1.set_title("Scripted Window-Management Language by Scenario\n"
    ...
    if scen_scripted["repeat_31_60d"].notna().any():
        ...
        ax2.legend(loc="lower right", facecolor="#16213e", edgecolor=CHART_FG, fontsize=9)
    ax1.legend(loc="upper left", facecolor="#16213e", edgecolor=CHART_FG, fontsize=9)
    ax1.grid(True, axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(fig_dir / "scripted_language_by_scenario.png", dpi=CHART_DPI)
    plt.close()
...
def write_summary_report(df, rep_summary, scenario_summary, monthly_trends,
                         ...
                         min_calls_for_rank=20, paper_signals=None):
    n = len(df)
    ...
    lines += ["INTERPRETATION GUIDE",
              ...
              "  - Rep DAR = rep-level F/D (paper formula). Identifies which reps drive 31-60d callbacks.",
...
def cmd_run(args):
    ...
    print("\n" + "="*60)
    print("TRUST SIGNAL HEALTH PIPELINE v3.0")
    print("="*60)
    ...
    # Paper governance signals (Aulabaugh 2026) — DAR, DRL, DOV, POR, TER, SII
    print("Computing paper governance signals (Aulabaugh 2026)...")
    ...
    print(f"  DAR={ps['component_scores']['DAR']:.4f}  DRL={ps['component_scores']['DRL']:.4f}{drl_note}  "
          f"DOV={ps['component_scores']['DOV']:.4f}  POR={ps['component_scores']['POR']:.4f}{por_note}")
    print(f"  SII (gated): {ps['SII_gated']:.1f} → {sii_status}")
    if paper_signals["TER"]["value"] is not None:
        ter_val = paper_signals["TER"]["value"]
        base_churn = paper_signals["TER"].get("baseline_churn")
        print(f"  TER: {ter_val:.4f}  baseline churn: {base_churn if base_churn is not None else 'N/A'}")
    if sii_status == "VETO":
        print("  ⛔ SII VETO — halt AI optimization, re-audit measurement environment")
    elif sii_status == "WATCH":
        print("  ⚠  SII WATCH — human review required before next optimization cycle")
    print("")

    # Charts (12 total: 9 original + 3 new)
    ...
    print("  12 charts written to output/figures/\n")
    ...
    gov_report = build_governance_report(
        ...
        monthly_trends=monthly_trends, alert_summary=alert_summary,
        churn_decile=churn_decile, df_scored=df_scored, paper_signals=paper_signals)
    ...
    print("    churn_by_trust_decile.png    rep_dar_ranking.png")
    print("    scenario_dov.png             scripted_language_by_scenario.png")
    print("  output/reports/")
    print("    integrity_summary.json       summary_report.txt")
    print("    governance_report.json       threshold_alerts.json")
    print("    threshold_alerts.txt         paper_signals.json")
    print("")
    ...
    print(f"SII: {ps['SII_gated']:.1f} [{sii_status}]  "
          f"DAR={ps['component_scores']['DAR']:.3f}  DRL={ps['component_scores']['DRL']:.3f}{drl_note}  "
          f"DOV={ps['component_scores']['DOV']:.3f}  POR={ps['component_scores']['POR']:.3f}{por_note}")
    print("Done.")
    return 0
...
def build_parser():
    p = argparse.ArgumentParser(description="NovaWireless Trust Signal Health Pipeline v3.0 (Aulabaugh 2026 signals)")
    p.add_argument("--dupe_policy", default="quarantine_extras_keep_latest",
